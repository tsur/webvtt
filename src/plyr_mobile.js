...
;(function(root, factory) {
    ...
    defaults = {
        ...
        types: {
            embed:              ['youtube', 'vimeo', 'soundcloud', 'youtube-cap'],
            html5:              ['video', 'audio']
        },
        // URLs
        urls: {
    ...
    function Plyr(container, config) {
        ...
        // Setup captions
        function _setupCaptions(media) {

            if (plyr.type !== 'video' && plyr.type !== 'youtube-cap') {
                return;
            }

            // Inject the container
            if (!_getElement(config.selectors.captions)) {
              (plyr.type === 'youtube-cap' ? plyr.container : plyr.videoContainer).insertAdjacentHTML('afterbegin', '<div class="' + _getClassname(config.selectors.captions) + '"></div>');
            }

            // Determine if HTML5 textTracks is supported
            plyr.usingTextTracks = false;
            if ((media || plyr.media).textTracks) {
            ...
            var captionSrc = '',
                kind,
                children = (media || plyr.media).childNodes;
            ...
            // If caption file exists, process captions
                ...
                var tracks = (media || plyr.media).textTracks;
        ...
        function _seekManualCaptions(time, media) {
            ...
            if (plyr.usingTextTracks || plyr.type !== 'video' || plyr.type !== 'youtube-cap' || !plyr.supported.full) {
                return;
            }
            ...
            if ((media||plyr.media).currentTime.toFixed(1) >= _timecodeMin(plyr.captions[plyr.subcount][0]) &&
              (media||plyr.media).currentTime.toFixed(1) <= _timecodeMax(plyr.captions[plyr.subcount][0])) {
        ...
        function _insertChildElements(type, attributes, media) {
            if (typeof attributes === 'string') {
               _insertElement(type, media || plyr.media, { src: attributes });
            }
            else if (attributes.constructor === Array) {
                for (var i = attributes.length - 1; i >= 0; i--) {
                    _insertElement(type, media || plyr.media, attributes[i]);
                }
            }
        }
        ...
        function _setupEmbed() {
            ...
            // YouTube
            if (plyr.type === 'youtube' || plyr.type === 'youtube-cap') {
                ...
                events: {
                    ...
                    'onReady': function(event) {
                        ...
                        plyr.timer.buffering = window.setInterval(function() {
                            ...
                            if (plyr.media.buffered === 1 && !plyr.mediaCapLoading) {
                                window.clearInterval(plyr.timer.buffering);
        ...
        function _updateSource(source) {
          ...
          if(plyr.type === 'youtube-cap'){
            ...
            if ('tracks' in source) {
              _insertChildElements('track', source.tracks, plyr.mediaCap);
              _setupCaptions(plyr.mediaCap);
            }
